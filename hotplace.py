...
def get_list():
    ...
    url = 'http://piao.qunar.com/ticket/list.htm?keyword=' + \
          str(place) + '&region=&from=mpl_search_suggest&page={}'
    i = 1
    sightlist = []
    while i < 3:
        ...
        for inf in informations:  # 获取必要信息
            ...
            sight_hot = inf.xpath(
                './/span[@class="product_star_level"]//span/text()')[0].replace('热度 ', '')
            sight_add = inf.xpath(
                './/p[@class="address color999"]/span/text()')[0]
            sight_add = re.sub(
                '地址：|（.*?）|\(.*?\)|，.*?$|\/.*?$', '', str(sight_add))
            sight_slogen = inf.xpath(
                './/div[@class="intro color999"]/text()')[0]
            ...
            sightlist.append([sight_name, sight_level, sight_area, float(sight_price), int(
                sight_soldnum), float(sight_hot), sight_add.replace('地址：', ''), sight_slogen, sight_url])
        # time.sleep(1)
    return sightlist, place


def list_to_excel(list, name):
    df = pd.DataFrame(
        list, columns=['景点名称', '级别', '所在区域', '起步价', '销售量', '热度', '地址', '标语', '详情网址'])
    df.to_excel(name + '景点信息.xlsx')


def get_baidu_geo(sightlist, name):
    ak = '你的百度密钥'
    headers = {
...
def main():
    sightlist, place = get_list()
    list_to_excel(sightlist, place)
    get_baidu_geo(sightlist, place)
